...
def download_order(request, order_id=None, *args, **kwargs):
    ...
    qs = Order.objects.filter(id=order_id, user=request.user, status='paid', product__media__isnull=False) # Esta es la razon por la cual tenemos Foreign Keys
    if not qs.exists():
        return redirect("/orders")
    order_obj = qs.first()
    ...
    if not product_obj.media:
        return redirect("/orders")
    media = product_obj.media # Hacemos un trabajito para cambiarle el nombre al producto, para que cuando lo descargue tenga un mejor nombre
